...
def calculateRate(include=['allCells', 'eachPop'], peakBin=5, timeRange=None):
    ...
    for iplot, subset in enumerate(include):
        ...
        if peakBin:
            ...
            histoCount = (
                histoCount * float((1000.0 / peakBin)) / float((len(cellGids) + numNetStims))
            )  # convert to firing rate
            peak.append(float(max(histoCount)))

        spktsRange = [spkt for spkt in spkts if timeRange[0] <= spkt <= timeRange[1]]
        avg.append(
            float(len(spktsRange))
            / float((len(cellGids) + numNetStims))
            / float((timeRange[1] - timeRange[0]))
            * 1000.0
        )
...
def plotRates(
    ...
    if not timeRangeLabels:
        timeRangeLabels = [
            '%f-%f ms' % (t[0], t[1]) for t in timeRanges
        ]  # ['period '+i for i in range(len(timeRanges))]
...
def plotSyncs(
    ...
    for i, timeRange in enumerate(timeRanges):
        print(timeRange)
        _, sync = sim.analysis.plotSpikeStats(
            include=include, timeRange=timeRange, stats=['sync'], saveFig=False, showFig=False
        )
        print(sync)
...
def plotRaster(
    ...
    if (
        isinstance(orderBy, basestring) and orderBy not in cells[0]['tags']
    ...
    sel.plot.scatter(
        ax=ax1, x='spkt', y='spkind', lw=lw, s=markerSize, marker=marker, c=sel['spkgidColor'].tolist()
    )  # Create raster
    ...
    if syncLines:
        ...
        plt.title(
            'cells=%i   syns/cell=%0.1f   rate=%0.1f Hz   sync=%0.2f'
            % (numCells, connsPerCell, firingRate, syncMeasure()),
            fontsize=fontsiz,
        )
    else:
        plt.title(
            'cells=%i   syns/cell=%0.1f   rate=%0.1f Hz' % (numCells, connsPerCell, firingRate), fontsize=fontsiz
        )
    # Axis
    ...
    if popRates:
        popLabelRates = [
            popLabel + ' (%.3g Hz)' % (avgRates[popLabel]) for popLabel in popLabels if popLabel in avgRates
        ]
    ...
    if labels == 'legend':
        ...
        plt.legend(fontsize=fontsiz, bbox_to_anchor=(1.04, 1), loc=2, borderaxespad=0.0)
        ...
        if len(tysOffset):
            tysOffset = [tysOffset[0]] + [
                tysOffset[i] + max(tysOffset[i + 1] - tysOffset[i], minSpacing) for i in range(len(tysOffset) - 1)
            ]
...
def plotRatePSD(
    ...
    for iplot, (subset, freqs, signal) in enumerate(zip(include, allFreqs, allSignal)):
        color = (
            popColors[subset]
            if isinstance(subset, (str, tuple)) and subset in popColors
            else colorList[iplot % len(colorList)]
        )
    ...
    if overlay:
        for i, subset in enumerate(include):
            color = (
                popColors[subset]
                if isinstance(subset, basestring) and subset in popColors
                else colorList[i % len(colorList)]
            )
...
def plotRateSpectrogram(
    ...
    for iplot, (subset, freqs, signal) in enumerate(zip(include, allFreqs, allSignal)):
        ...
        plt.imshow(
            signal,
            extent=(np.amin(T), np.amax(T), np.amin(freqs), np.amax(freqs)),
            origin='lower',
            interpolation='None',
            aspect='auto',
...
def calculatefI():
    ...
    sim.allSimData['fI'] = [
        len([spkt for spkt in sim.allSimData['spkt'] if t <= spkt < t + dur]) / (dur / 1000.0) for t in times
    ]
    ...
    if durSteady:  # rate based on the last 'dur' ms
        sim.allSimData['fI_steady'] = [
            len([spkt for spkt in sim.allSimData['spkt'] if t + dur - durSteady <= spkt < t + dur])
            / (durSteady / 1000.0)
            for t in times
        ]
    ...
    peakHeights = []
    for time in times:
        ptimes, properties = find_peaks(
            v[int(time / dt) : int((time + dur) / dt)], height=threshold, distance=minDistance
        )
