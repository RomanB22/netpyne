...
def plotConn(
    ...
    if connsFile and tagsFile:
        connMatrix, pre, post = _plotConnCalculateFromFile(
            includePre,
            includePost,
            feature,
            orderBy,
    ...
    if graphType == 'matrix':
        ...
        h = plt.axes()
        plt.imshow(
            connMatrix, interpolation='nearest', cmap='viridis', vmin=np.nanmin(connMatrix), vmax=np.nanmax(connMatrix)
        )  # _bicolormap(gap=0)
        ...
        if groupBy == 'cell':
            ...
            for igroup, group in enumerate(groupsPost):
                plt.plot(
                    np.array([igroup, igroup]) - 0.5, np.array([0, len(groupsPost)]) - 0.5, '-', c=(0.7, 0.7, 0.7)
                )
...
def plot2Dnet(
        ...
        for postCell in cells:
            for con in postCell['conns']:  # plot connections between cells
                if not isinstance(con['preGid'], basestring) and con['preGid'] in cellGids:
                    ...
                    plt.plot(
                        [posXpre, posXpost], [posYpre, posYpost], color=color, linewidth=width
    ...
    plt.legend(fontsize=fontsiz, bbox_to_anchor=(1.01, 1), loc=2, borderaxespad=0.0)
    ax = plt.gca()
    ax.invert_yaxis()
        ...
        out = list(
            zip(
                *[
                    ...
                    if timeInterval[0] <= spkt <= timeInterval[1]
                ]
            )
        )
        if len(out) == 2:
...
def calculateDisynaptic(
    includePost=['allCells'],
    includePre=['allCells'],
    includePrePre=['allCells'],
    tags=None,
    conns=None,
    tagsFile=None,
    connsFile=None,
    ...
    if tags and conns:
        ...
        for postGid in cellsPostGids:
            preGidsAll = [
                conn[preGidIndex]
                for conn in conns[postGid]
                if isinstance(conn[preGidIndex], Number) and conn[preGidIndex] in cellsPreGids + cellsPrePreGids
            ]
    ...
    print(
        '    Total disynaptic connections: %d / %d (%.2f%%)'
        % (numDis, totCon, float(numDis) / float(totCon) * 100 if totCon > 0 else 0.0)
    )
    try:
