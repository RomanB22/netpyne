    ...
    **kwargs
    ...
    for netStimLabel in netStimLabels:
        ...
        if len(netStimSpks) > 0:
            ...
            spikesNew = [
                (spkt, spkindsNew[icell_stim])
                for icell_stim, cell_stim in enumerate(netStimSpks)
                for spkt in cell_stim
            ]
    ...
    if cells:
        ...
        popNumConns = [
            sum([cellNumConn for cellIndex, cellNumConn in enumerate(cellNumConns) if gidPops[cellIndex] == pop])
        ...
        popConnsPerCell = [
            popNumConns[popIndex] / popNumCells[popIndex] if popNumConns[popIndex] > 0 else 0
            for popIndex, pop in enumerate(popLabels)
        ]
    ...
    if popRates:
        ...
        if numCells > 0:
            if popRates == 'minimal':
                legendLabels = [
                    popLabel + ' (%.3g Hz)' % (avgRates[popLabel])
                    for popIndex, popLabel in enumerate(popLabels)
                    if popLabel in avgRates
                ]
                ...
                legendLabels = [
                    popLabel
                    + '\n  cells: %i\n  syn/cell: %0.1f\n  rate: %.3g Hz'
                    % (popNumCells[popIndex], popConnsPerCell[popIndex], avgRates[popLabel])
                    for popIndex, popLabel in enumerate(popLabels)
                    if popLabel in avgRates
                ]
                title = 'cells: %i   syn/cell: %0.1f   rate: %0.1f Hz' % (numCells, connsPerCell, firingRate)
    ...
    axisArgs = {'xlabel': 'Time (ms)', 'ylabel': ylabelText, 'title': title}

    spikeData = {
        'spkTimes': sel['spkt'].tolist(),
        'spkInds': sel['spkind'].tolist(),
        'spkGids': sel['spkid'].tolist(),
        'popNumCells': popNumCells,
        'popLabels': popLabels,
