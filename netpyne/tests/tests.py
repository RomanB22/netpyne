...
class TestTypeObj(object):
    ...
    def testExistsInList(self, val, params):
        try:
            assert any([x in params for x in val]), (
                " At least one of " + str(val) + " must be specified in " + str(params) + "."
            )
        except AssertionError as e:
            e.args += (val,)
            raise
        ...
        try:
            ...
            if isinstance(paramValues[val], dict) and 'pop' in paramValues[val]:
                ...
                if isinstance(pops, dict) and paramValues[val]['pop'] not in list(pop.keys()):
                    errorMessage = (
                        "ConnParams->'pop': Pop specified in conn params is: "
                        + str(paramValues[val]['pop'])
                        + ". This does not exist in list of pop keys = "
                        + str(list(pop.keys()))
                        + "."
                    )
                    return errorMessage
    ...
    def testIsAllValueList(self, val, valList):  # TEST_TYPE_ALL_VALUE_LIST
        try:
            ...
            if isinstance(val, list) and isinstance(valList, list):
                assert all([x in valList for x in val]), (
                    str(val[[x in valList for x in val].index(False)]) + " must be in list " + str(valList)
                )
        except AssertionError as e:
            # print (" 1" + str(e))
            # e.args += (,)
    ...
    def testDictKeyValidValue(self, paramDict, valList):  # TEST_TYPE_VALUE_LIST
        try:
            ...
            if isinstance(paramDict, dict) and isinstance(valList, list):
                ...
                assert all([x in valList for x in list(paramDict.keys())]), (
                    " contains invalid key '"
                    + str(list(paramDict.keys())[[x in valList for x in list(paramDict.keys())].index(False)])
                    + "': Valid values are: "
                    + str(valList)
                    + "."
                )
            ...
            if not stimParamsAllowed:
                if any([x in POP_STIM_KEYS for x in paramValues]):
                    ...
                    errorMessages.append(
                        "popParams: Any or all of the params '"
                        + str(POP_STIM_KEYS)
                        + "' are allowed only if cellModel in '"
                        + str(['IntFire1', 'IntFire2', 'NetStim', 'VecStim'])
                        + "'."
                    )
                ...
                if 'interval' in paramValues:
                    if not isinstance(paramValues['interval'], numbers.Real):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'interval': Must be a float if specified. Value provided is:"
                            + str(paramValues['interval'])
                        )
                if 'rate' in paramValues:
                    if not isinstance(paramValues['rate'], numbers.Real):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'rate': Must be a float if specified. Value provided is:"
                            + str(paramValues['rate'])
                        )
                if 'noise' in paramValues:
                    if not isinstance(paramValues['noise'], numbers.Real):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'noise': Must be a float between 0 and 1 if specified. Value provided is:"
                            + str(paramValues['noise'])
                        )
                ...
                if 'start' in paramValues:
                    if not isinstance(paramValues['start'], numbers.Real):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'start': Must be a float if specified. Value provided is:"
                            + str(paramValues['start'])
                        )
                if 'number' in paramValues:
                    if not isinstance(paramValues['number'], numbers.Real):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'number': Must be a float if specified. Value provided is:"
                            + str(paramValues['number'])
                        )
                if paramValues['cellModel'] != 'VecStim' and any([x in paramValues for x in ['spkTimes', 'pulses']]):
                    stimValid = False
                    errorMessages.append(
                        "popParams: 'spkTimes' or 'pulses' can be provided if 'cellModel' is 'VecStim' "
                    )
                if 'spkTimes' in paramValues:
                    if not isinstance(paramValues['spkTimes'], list):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'spkTimes': Must be a list if specified. Value provided is:"
                            + str(paramValues['spkTimes'])
                        )
                if 'pulses' in paramValues:
                    ...
                    if not isinstance(paramValues['pulses'], list):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'pulses': Must be a list of dicts if specified. They keys for each dict are '"
                            + str(PULSE_KEYS)
                            + "' Value provided is:"
                            + str(paramValues['pulses'])
                        )
                    if not all([isinstance(x, dict) for x in paramValues['pulses']]):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'pulses': Must be a list of dicts if specified. Value provided is:"
                            + str(paramValues['pulses'])
                        )
                ...
                for key, values in list(paramValues['secs'].items()):
                    if 'geom' in values:
                        if len(values['geom']) == 0:
                            errorMessage = (
                                "cellParams -> secs ('" + str(key) + "'): Geom parameters must be specified."
                            )
                            geomValid = False
                        if not isinstance(values['geom'], dict):
                            errorMessage = (
                                "cellParams -> secs ('" + str(key) + "'): Geom parameters must be specified as a dict."
                            )
                            geomValid = False
                        ...
                        for key1, values1 in list(values['geom'].items()):
                            ...
                            if key1 not in VALID_GEOMETRIES:
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> 'geom': Invalid geom parameter '"
                                    + str(key1)
                                    + "' specified. Valid values are '"
                                    + str(VALID_GEOMETRIES)
                                    + "'."
                                )
                                geomValid = False
                            if not geomValid:
                            ...
                            if PT_3D in values['geom']:
                                if not isinstance(values['geom'][PT_3D], list) and not isinstance(
                                    values['geom'][PT_3D], tuple
                                ):
                                    errorMessage = (
                                        "cellParams -> secs ('"
                                        + str(key)
                                        + "') -> 'geom': pt3D must be an array with each array element being a 4-element list or array of floats."
                                    )
                                    geomValid = False
                                    ...
                                    errorMessage = (
                                        "cellParams -> secs ('"
                                        + str(key)
                                        + "') -> 'geom': At least one element must be provided for pt3D."
                                    )
                                    geomValid = False
                                ...
                                for elem in values['geom'][PT_3D]:
                                    if not isinstance(elem, list) and not isinstance(elem, tuple):
                                        errorMessage = (
                                            "cellParams -> secs ('"
                                            + str(key)
                                            + "') -> 'geom' -> 'pt3D':Type error. pt3D must be an array with each array element being a 4-element list or array of floats.Value specified is: '"
                                            + str(elem)
                                            + "'."
                                        )
                                        geomValid = False
                                        ...
                                        errorMessage = (
                                            "cellParams -> secs ('"
                                            + str(key)
                                            + "') -> 'geom' -> 'pt3D':Length error. pt3D must be an array with each array element being a 4-element list or array of floats.Value specified is: '"
                                            + str(elem)
                                            + "'."
                                        )
                                        geomValid = False
                                    ...
                                    for elem2 in elem:
                                        if not isinstance(elem2, numbers.Real):
                                            errorMessage = (
                                                "cellParams -> secs ('"
                                                + str(key)
                                                + "') -> 'geom' -> 'pt3D':Float error. pt3D must be an array with each array element being a 4-element list or array of floats. Value specified is: '"
                                                + str(elem2)
                                                + "'."
                                            )
                                            geomValid = False
                    ...
                    for key, value in list(paramValues['secs'].items()):
                        if 'topol' not in value:
                            if topolNeeded:
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(list(paramValues['secs'].keys()))
                                    + "'): Topology needs to be specified if more than one section."
                                )
                        else:
                            topolValid = True
                            ...
                            if not isinstance(value['topol'], dict):
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol: Topology, if specified, must be a dict. Value specified is '"
                                    + str(value['topol'])
                                    + "'."
                                )
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol: At least 3 parameters (parentSec, parentX and childX) must be specified for topology. Values specified are: '"
                                    + str(list(value['topol'].keys()))
                                    + "'."
                                )
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol: Invalid value specified:''"
                                    + str(value['topol'])
                                    + "'. Valid values are: '"
                                    + str(VALID_TOPOLOGY_PARAMS)
                                    + "'."
                                )
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol -> parentSec: parentSec '"
                                    + str(value['topol']['parentSec'])
                                    + "' does not point to a valid section. Valid sections are ('"
                                    + str(list(paramValues['secs'].keys()))
                                    + "')."
                                )
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol -> parentX: Value should be a float."
                                )
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol -> childX: Value should be a float."
                                )
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol -> parentX: parentX must be between 0 and 1. Value specified is '"
                                    + str(value['topol']['parentX'])
                                    + "'."
                                )
                                ...
                                errorMessage = (
                                    "cellParams -> secs ('"
                                    + str(key)
                                    + "') -> topol -> parentX: childX must be between 0 and 1. Value specified is '"
                                    + str(value['topol']['childX'])
                                    + "'."
                                )
                                    ...
                                    if key2.find("_ion") != -1:
                                        ...
                                        if not any([x in mechs[key2] for x in list(values2.keys())]):
                                            ...
                                            errorMessage = (
                                                ...
                                                + str(ionName)
                                                + "'): Invalid ions ("
                                                + str(list(values2.keys()))
                                                + ") specified. Valid value are: "
                                                + str(mechs[key2])
                                        ...
                                        if not all([x in mechs[key2] for x in keys_suffixed]):
                                            ...
                                            errorMessage = (
                                                ...
                                                + "') -> mechs ('"
                                                + str(key2)
                                                + "'): Incomplete list provided. Complete list is: "
                                                + str(mechs_unsuffixed)
                                                + ". Values specified are "
                                                + str(list(values2.keys()))
                                                + "."
                                    ...
                                    if 'mod' not in list(values2.keys()):
                                        ...
                                        if not isinstance(loc, numbers.Real):
                                            errorMessages.append(
                                                ...
                                                + "') -> pointps ('"
                                                + str(key2)
                                                + "'): Loc must be a float. Value provided is "
                                                + str(loc)
                                            )
                                        elif loc < 0 or loc > 1:
                                            errorMessages.append(
                                                ...
                                                + "') -> pointps ('"
                                                + str(key2)
                                                + "'): Loc must be a between 0 and 1. Value provided is "
                                                + str(loc)
                                            )
                                        ...
                                        if not isinstance('synList', list):
                                            errorMessages.append(
                                                ...
                                                + "') -> pointps ('"
                                                + str(key2)
                                                + "'): SynList must be a list. Value provided is "
                                                + str(values2['synList'])
                                            )
                ...
                if synsPerConn != 1:
                    if dimValues not in [0, 1]:
                        errorMessage = (
                            "ConnParams -> '"
                            + str(paramValue)
                            + "' can only be a number, function or 1d list if only one 1 synMech and synsPerConn > 1."
                        )
                        synMechsValid = False
                        ...
                        errorMessage = (
                            ...
                            + "': Dimension of "
                            + str(paramValue)
                            + " locs array must be same as synsPerConn."
                        )
                        synMechsValid = False
                    ...
                    if dimValues != 0:
                        errorMessage = (
                            "ConnParams -> '"
                            + str(paramValue)
                            + "' can only be a number if 1 synMech and synsPerConn = 1."
                        )
                        synMechsValid = False
                ...
                if synsPerConn != 1:
                    if dimValues not in [0, 1, 2]:
                        errorMessage = (
                            "ConnParams -> '"
                            + str(paramValue)
                            + "' can only be a number or 1d or 2D list if more than 1 synMech and synsPerConn > 1."
                        )
                        synMechsValid = False
                        ...
                        if numpy.array(locs).shape[1] != synsPerConn:
                            ...
                            errorMessage = (
                                ...
                                + "': Invalid "
                                + str(paramValue)
                                + " for synMechs and synsPerConn. If specifying 2D array, please ensure that the array of arrays has the number of elements as # of synMechs, with each array having the number of elements as specified by synsPerConn."
                            )
                            synMechsValid = False
                else:  # only 1 synsPerConn
                    if dimValues not in [0, 1]:
                        errorMessage = (
                            "ConnParams -> "
                            + str(paramValue)
                            + " can only be a number or 1 D list if more than 1 synMech and synsPerConn = 1."
                        )
                        synMechsValid = False
                    elif dimValues == 1 and len(values) != dimSynMechs:
                        errorMessage = (
    ...
    def testValidSynMechs(self, parameterName, paramValues, netParams):  # TEST_TYPE_VALID_SYN_MECHS
        ...
        try:
            ...
            if parameterName in paramValues:
                ...
                if parameterName == 'loc':
                    ...
                    if not all([isinstance(x, numbers.Real) for x in flattenedValues]):
                        errorMessage = (
                            "ConnParams -> " + str(parameterName) + " must be a number with values between 0 and 1."
                        )
                        synMechsValid = False
            ...
            # print ( " in check 1 synsPerConn 222 dimSynMechs = " + str(dimSynMechs) + " synsPerConn = " + str(synsPerConn))
            errorMessage, synMechsValid = self.checkSyncMechs(
                parameterName, values, dimValues, dimSynMechs, synsPerConn
            )
                    ...
                    for key, value in list(secList.items()):
                        ...
                        if not isinstance(value, list):
                            errorMessage = (
                                "cellParams -> secList ('"
                                + str(key)
                                + "'):Each element of seclist must be a list. Value specified is: "
                                + str(value)
                                + "."
                            )
                            ...
                            errorMessage = (
                                ...
                                + "'): "
                                + str(value)
                                + " - Sections specified in secList keys must be specified in cells. Valid list is "
                                + str(validSections)
                                + "."
                            )
        ...
        try:
            ...
            if pulseType != '':
                if pulseType not in ['square', 'gaussian']:
                    errorMessage = (
                        "connList -> shape: Pulse type, if specified, can only be square or gaussian. Value specified is "
                        + str(pulseType)
                        + "."
                    )
                    errorMessages.append(errorMessage)
            ...
            if pulsePeriod != '':
                if not isinstance(pulsePeriod, numbers.Real):
                    errorMessage = (
                        "connList -> shape: Pulse period, if specified, must be a float. Value specified is "
                        + str(pulsePeriod)
                        + "."
                    )
                    errorMessages.append(errorMessage)
            ...
            if pulseWidth != '':
                if not isinstance(pulseWidth, numbers.Real):
                    errorMessage = (
                        "connList -> shape: Pulse width, if specified, must be a float. Value specified is "
                        + str(pulseWidth)
                        + "."
                    )
                    errorMessages.append(errorMessage)
    ...
    def testValidConnPlasticity(self, paramValues):  # TEST_TYPE_CONN_PLASTICITY
        ...
        try:
            ...
            if 'mech' not in plasticity:
                errorMessage = (
                    "connParams -> 'plasticity':'mech' must be specified in plasticity in connParams with label."
                )
                errorMessages.append(errorMessage)
    ...
    def testValidStimSource(self, paramValues):  # TEST_TYPE_STIM_SOURCE_TEST
        ...
        try:
            ...
            if simType not in validTypes:
                errorMessage = (
                    " StimSourceParams -> type: Invalid simtype "
                    + str(simType)
                    + ". Valied values are: "
                    + str(validTypes)
                    + "."
                ...
                if any([x not in allowedValues for x in allKeys]):
                    errorMessage = (
                        "StimSourceParams: Invalid parameter specified. Values specified are "
                        + str(allKeys)
                        + ", while allowed values are: "
                        + str(allowedValues)
                    )
                    errorMessages.append(errorMessage)
    ...
    def testValidStimTarget(self, paramValues, netParams):  # TEST_TYPE_STIM_TARGET_TEST
        ...
        try:
            if 'source' not in paramValues:
                ...
                errorMessage = (
                    "StimTargetParams -> source:"
                    + str(paramValues['source'])
                    + ": Invalid source specified in stimTargetParams. The source must exist in stimSourceParams."
                )
                errorMessages.append(errorMessage)
                return errorMessages
        ...
        try:
            ...
            if simConfig and simConfig.analysis:
                ...
                if not isinstance(simConfig.analysis, dict):
                    errorMessages.append(
                        "SimConfig->'analysis': Must be a dict. Value provided is " + str(simConfig.analysis) + "."
                    )
                    return errorMessages
                # print (" before ")
                validList = [
                ...
                if not all([x in validList for x in list(analysis.keys())]):
                    errorMessages.append(
                        "SimConfig->'analysis': Valid analysis functions are 'plotRaster','plotSpikeHist', 'plotTraces', 'plotShape', 'plotConn', 'plot2DNet', 'nTE', 'granger', 'plotSpikeStats'. Keys specified are "
                        + str(list(analysis.keys()))
                        + "."
                    )
                # print (" after ")
                ...
                if 'plotRaster' in analysis:
                    ...
                    if not isinstance(plotRaster, dict) and not isinstance(plotRaster, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'plotRaster': Must be a dict or bool.  Value provided is "
                            + str(plotRaster)
                            + "."
                        )

                    elif isinstance(plotRaster, dict):
                        ...
                        if not all(x in validList for x in list(plotRaster.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotRaster': plotRaster must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(plotRaster.keys()))
                                + "."
                            )

                        if 'include' in plotRaster and not isinstance(plotRaster['include'], dict):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotRaster'->'include': Must be a list. Value provided is "
                                + str(plotRaster['include'])
                                + "."
                            )
                        ...
                        if 'maxSpikes' in plotRaster and not isinstance(plotRaster['maxSpikes'], int):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotRaster'->'maxSpikes': Must be an integer. Value provided is "
                                + str(plotRaster['maxSpikes'])
                                + "."
                            )
                        ...
                        if 'orderBy' in plotRaster:
                            ...
                            if plotRaster['orderBy'] not in ['gid', 'ynorm', 'y']:
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotRaster'->'orderBy': Valid values are "
                                    + str(['gid', 'ynorm', 'y'])
                                    + ". Value provided is "
                                    + str(plotRaster['orderBy'])
                                    + "."
                                )
                        ...
                        if 'orderInverse' in plotRaster:
                            ...
                            if not isinstance(plotRaster['orderInverse'], bool):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotRaster'->'orderInverse': Must be boolean. Value provided is "
                                    + str(plotRaster['orderInverse'])
                                    + "."
                                )
                        ...
                        if 'labels' in plotRaster:
                            ...
                            if any([x not in ['legend', 'overlay', 'y'] for x in plotRaster['labels']]):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotRaster'->'labels': Valid values are "
                                    + str(['legend', 'overlay'])
                                    + ". Value provided is "
                                    + str(plotRaster['labels'])
                                    + "."
                                )

                        if 'popRates' in plotRaster:

                            if not isinstance(plotRaster['popRates'], basestring) and not isinstance(
                                plotRaster['popRates'], bool
                            ):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotRaster'->'popRates': Must be a string or boolean. Value provided is "
                                    + str(plotRaster['popRates'])
                                    + "."
                                )
                            else:
                                if (isinstance(plotRaster['popRates'], basestring)) and (
                                    x not in ['legend', 'overlay']
                                ):
                                    errorMessages.append(
                                        "SimConfig->'analysis'->'plotRaster'->'popRates': Valid values are "
                                        + str(['legend', 'overlay'])
                                        + ". Value provided is "
                                        + str(plotRaster['labels'])
                                        + "."
                        ...
                        if 'spikeHist' in plotRaster:
                            ...
                            if not isinstance(plotRaster['spikeHist'], list):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotRaster'->'spikeHist': Must be a list. Value provided is "
                                    + str(plotRaster['spikeHist'])
                                    + "."
                                )
                            else:
                                if any([x not in ['legend', 'overlay', 'y'] for x in plotRaster['spikeHist']]):
                                    errorMessages.append(
                                        "SimConfig->'analysis'->'plotRaster'->'spikeHist': Valid values are "
                                        + str(['legend', 'overlay'])
                                        + ". Value provided is "
                                        + str(plotRaster['labels'])
                                        + "."
                ...
                if 'plotSpikeHist' in analysis:
                    ...
                    if not isinstance(plotSpikeHist, dict) and not isinstance(plotSpikeHist, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'plotSpikeHist': Must be a dict or bool.  Value provided is "
                            + str(plotSpikeHist)
                            + "."
                        )

                    elif isinstance(plotSpikeHist, dict):

                        if 'include' in plotSpikeHist and not isinstance(plotSpikeHist['include'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikeHist'->'include': Must be a list. Value provided is "
                                + str(plotSpikeHist['include'])
                                + "."
                            )
                        ...
                        if 'orderInverse' in plotSpikeHist:

                            if not isinstance(plotSpikeHist['orderInverse'], bool) and not isinstance(
                                plotSpikeHist, dict
                            ):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotSpikeHist'->'orderInverse': Must be boolean. Value provided is "
                                    + str(plotSpikeHist['orderInverse'])
                                    + "."
                                )

                        if 'overlay' in plotSpikeHist:

                            if not isinstance(plotSpikeHist['overlay'], bool):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotSpikeHist'->'overlay': Must be boolean. Value provided is "
                                    + str(plotSpikeHist['overlay'])
                                    + "."
                                )

                        if 'graphType' in plotSpikeHist and plotSpikeHist['graphType'] not in ['line', 'bar']:
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikeHist'->'graphType': GraphType must be in "
                                + str(['line', 'bar'])
                                + ". Value provided is "
                                + str(plotSpikeHist['graphType'])
                                + "."
                            )

                        if 'yaxis' in plotSpikeHist and plotSpikeHist['yaxis'] not in ['rate', 'count']:
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikeHist'->'yaxis': yaxis must be in "
                                + str(['rate', 'count'])
                                + ". Value provided is "
                                + str(plotSpikeHist['yaxis'])
                                + "."
                            )

                        if 'figSize' in plotSpikeHist and not isinstance(plotSpikeHist['figSize'], tuple):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikeHist'->'figSize': figSize must be tuple if specified. Value provided is "
                                + str(plotSpikeHist['figSize'])
                                + "."
                            )

                        if 'binSize' in plotSpikeHist and not isinstance(plotSpikeHist['binSize'], int):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikeHist'->'binSize': Must be an integer. Value provided is "
                                + str(plotSpikeHist['binSize'])
                                + "."
                            )

                        if 'showFig' in plotSpikeHist:

                            if not isinstance(plotSpikeHist['showFig'], bool):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotSpikeHist'->'showFig': Must be boolean. Value provided is "
                                    + str(plotSpikeHist['showFig'])
                                    + "."
                                )
                ...
                if 'plotSpikePSD' in analysis:
                    ...
                    if not isinstance(plotSpikePSD, dict) and not isinstance(plotSpikePSD, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'plotSpikePSD': Must be a dict or bool.  Value provided is "
                            + str(plotSpikePSD)
                            + "."
                        )

                    elif isinstance(plotSpikePSD, dict):
                        ...
                        if not all(x in validList for x in list(plotSpikePSD.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikePSD': plotSpikePSD must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(plotSpikePSD.keys()))
                                + "."
                            )

                        if 'include' in plotSpikePSD and not isinstance(plotSpikePSD['include'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikePSD'->'include': Must be a list. Value provided is "
                                + str(plotSpikePSD['include'])
                                + "."
                            )
                        ...
                        if 'overlay' in plotSpikePSD:
                            ...
                            if not isinstance(plotSpikePSD['overlay'], bool):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotSpikePSD'->'overlay': Must be boolean. Value provided is "
                                    + str(plotSpikePSD['overlay'])
                                    + "."
                                )

                        if 'Fs' in plotSpikePSD and not isinstance(plotSpikePSD['Fs'], numbers.Real):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikePSD'->'Fs': Fs must be float. Value provided is "
                                + str(plotSpikePSD['Fs'])
                                + "."
                            )
                        ...
                        if 'figSize' in plotSpikePSD and not isinstance(plotSpikePSD['figSize'], tuple):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikePSD'->'figSize': figSize must be tuple if specified. Value provided is "
                                + str(plotSpikePSD['figSize'])
                                + "."
                            )

                        if 'binSize' in plotSpikePSD and not isinstance(plotSpikePSD['binSize'], int):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotSpikePSD'->'binSize': Must be an integer. Value provided is "
                                + str(plotSpikePSD['binSize'])
                                + "."
                            )
                        ...
                        if 'showFig' in plotSpikePSD:
                            ...
                            if not isinstance(plotSpikePSD['showFig'], bool):
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotSpikePSD'->'showFig': Must be boolean. Value provided is "
                                    + str(plotSpikePSD['showFig'])
                                    + "."
                                )
                ...
                if 'plotTraces' in analysis:
                    ...
                    if not isinstance(plotTraces, dict) and not isinstance(plotTraces, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'plotTraces': Must be a dict or bool.  Value provided is "
                            + str(plotTraces)
                            + "."
                        )

                    elif isinstance(plotTraces, dict):
                        ...
                        if not all(x in validList for x in list(plotTraces.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces': plotTraces must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(plotTraces.keys()))
                                + "."
                            )

                        if 'include' in plotTraces and not isinstance(plotTraces['include'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces'->'include': Must be a list. Value provided is "
                                + str(plotTraces['include'])
                                + "."
                            )
                        ...
                        if 'overlay' in plotTraces and not isinstance(plotTraces['overlay'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces'->'overlay': Must be boolean. Value provided is "
                                + str(plotTraces['overlay'])
                                + "."
                            )

                        if 'binSize' in plotTraces and not isinstance(plotTraces['binSize'], int):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces'->'binSize': Must be an integer. Value provided is "
                                + str(plotTraces['binSize'])
                                + "."
                            )

                        if 'oneFigPer' in plotTraces and plotTraces['oneFigPer'] not in ['cell', 'trace']:
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces'->'oneFigPer': oneFigPer must be in "
                                + str(['rate', 'count'])
                                + ". Value provided is "
                                + str(plotTraces['oneFigPer'])
                                + "."
                            )

                        if 'rerun' in plotTraces and not isinstance(plotTraces['rerun'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces'->'rerun': Must be boolean. Value provided is "
                                + str(plotTraces['rerun'])
                                + "."
                            )

                        if 'figSize' in plotTraces and not isinstance(plotTraces['figSize'], tuple):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces'->'figSize': figSize must be tuple if specified. Value provided is "
                                + str(plotTraces['figSize'])
                                + "."
                            )

                        if 'saveFig' in plotTraces and not isinstance(plotTraces['saveFig'], tuple):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotTraces'->'saveFig': saveFig must be tuple if specified. Value provided is "
                                + str(plotTraces['saveFig'])
                                + "."
                            )
                ...
                if 'plotShape' in analysis:
                    ...
                    if not isinstance(plotShapes, dict) and not isinstance(plotShapes, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'plotShape': Must be a dict or bool.  Value provided is "
                            + str(plotShape)
                            + "."
                        )
                        ...
                        if not all(x in validList for x in list(plotShape.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotShape': plotShape must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(plotShape.keys()))
                                + "."
                            )

                        if 'showSyns' in plotShapes and not isinstance(plotShape['showSyns'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotShape'->'showSyns': Must be boolean. Value provided is "
                                + str(plotShape['showSyns'])
                                + "."
                            )

                        if 'showFig' in plotShapes and not isinstance(plotShape['showFig'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotShape'->'showFig': Must be boolean. Value provided is "
                                + str(plotShape['showFig'])
                                + "."
                            )
                ...
                if 'plotConn' in analysis:
                    ...
                    if not isinstance(plotConn, dict) and not isinstance(plotConn, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'plotConn': Must be a dict or bool.  Value provided is "
                            + str(plotConn)
                            + "."
                        )

                    elif isinstance(plotConn, dict):
                        ...
                        if not all(x in validList for x in list(plotConn.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotConn': plotConn must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(plotConn.keys()))
                                + "."
                            )

                        if 'include' in plotConn and not isinstance(plotConn['include'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotConn'->'include': Must be a list. Value provided is "
                                + str(plotConn['include'])
                                + "."
                            )
                        ...
                        if 'feature' in plotConn:
                            if plotConn['spikeHist'] not in ['weight', 'delay', 'numConns']:
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotConn'->'feature': Valid values are "
                                    + str(['weight', 'delay', 'numConns'])
                                    + ". Value provided is "
                                    + str(plotConn['feature'])
                                    + "."
                                )
                        ...
                        if 'groupBy' in plotConn:
                            if plotConn['spikeHist'] not in ['pop', 'cell']:
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotConn'->'groupBy': Valid values are "
                                    + str(['pop', 'cell'])
                                    + ". Value provided is "
                                    + str(plotConn['groupBy'])
                                    + "."
                                )
                        ...
                        if 'orderBy' in plotConn:
                            if plotConn['spikeHist'] not in ['gid', 'ynorm', 'y']:
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plotConn'->'orderBy': Valid values are "
                                    + str(['gid', 'ynorm', 'y'])
                                    + ". Value provided is "
                                    + str(plotConn['orderBy'])
                                    + "."
                                )

                        if 'showFig' in plotConn and not isinstance(plotConn['showFig'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plotConn'->'showFig': Must be boolean. Value provided is "
                                + str(plotConn['showFig'])
                                + "."
                            )
                ...
                if 'plot2DNet' in analysis:
                    ...
                    if not isinstance(plot2DNet, dict) and not isinstance(plot2DNet, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'plot2DNet': Must be a dict or bool.  Value provided is "
                            + str(plot2DNet)
                            + "."
                        )
                    elif isinstance(plot2DNet, dict):
                        ...
                        if not all(x in validList for x in list(plot2DNet.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plot2DNet': plot2DNet must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(plot2DNet.keys()))
                                + "."
                            )

                        if 'include' in plot2DNet and not isinstance(plot2DNet['include'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plot2DNet'->'include': Must be a list. Value provided is "
                                + str(plot2DNet['include'])
                                + "."
                            )
                        ...
                        if 'orderBy' in plot2DNet:
                            if plot2DNet['spikeHist'] not in ['gid', 'ynorm', 'y']:
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plot2DNet'->'orderBy': Valid values are "
                                    + str(['gid', 'ynorm', 'y'])
                                    + ". Value provided is "
                                    + str(plot2DNet['orderBy'])
                                    + "."
                                )
                        ...
                        if 'view' in plot2DNet:
                            if plot2DNet['spikeHist'] not in ['xy', 'xz']:
                                errorMessages.append(
                                    "SimConfig->'analysis'->'plot2DNet'->'view': Valid values are "
                                    + str(['xy', 'xz'])
                                    + ". Value provided is "
                                    + str(plot2DNet['view'])
                                    + "."
                        ...
                        if 'showConns' in plot2DNet and not isinstance(plot2DNet['showConns'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plot2DNet'->'showConns': Must be boolean. Value provided is "
                                + str(plot2DNet['showConns'])
                                + "."
                            )

                        if 'showFig' in plot2DNet and not isinstance(plot2DNet['showFig'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plot2DNet'->'showFig': Must be boolean. Value provided is "
                                + str(plot2DNet['showFig'])
                                + "."
                            )
                ...
                if 'nTE' in analysis:
                    ...
                    if not isinstance(nTE, dict) and not isinstance(nTE, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'nTE': Must be a dict or bool.  Value provided is "
                            + str(nTE)
                            + "."
                        )
                        ...
                        if not all(x in validList for x in list(nTE.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'nTE': nTE must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(nTE.keys()))
                                + "."
                        ...
                        if 'cells1' in nTE and not isinstance(nTE['cells1'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'nTE'->'cells1': Must be a list. Value provided is "
                                + str(nTE['cells1'])
                                + "."
                            )
                        ...
                        if 'cells2' in nTE and not isinstance(nTE['cells2'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'nTE'->'cells2': Must be a list. Value provided is "
                                + str(nTE['cells2'])
                                + "."
                            )
                        ...
                        if 'spks1' in nTE and not isinstance(nTE['spks1'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'nTE'->'spks1': Must be a list. Value provided is "
                                + str(nTE['spks1'])
                                + "."
                            )
                        ...
                        if 'spks2' in nTE and not isinstance(nTE['spks2'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'nTE'->'spks2': Must be a list. Value provided is "
                                + str(nTE['spks2'])
                                + "."
                            )
                        ...
                        if 'binSize' in nTE and not isinstance(nTE['binSize'], int):
                            errorMessages.append(
                                "SimConfig->'analysis'->'nTE'->'binSize': Must be an int. Value provided is "
                                + str(nTE['binSize'])
                                + "."
                            )
                        ...
                        if 'numShuffle' in nTE and not isinstance(nTE['numShuffle'], int):
                            errorMessages.append(
                                "SimConfig->'analysis'->'nTE'->'numShuffle': Must be an int. Value provided is "
                                + str(nTE['numShuffle'])
                                + "."
                            )
                ...
                if 'granger' in analysis:
                    ...
                    if not isinstance(granger, dict) and not isinstance(granger, bool):
                        errorMessages.append(
                            "SimConfig->'analysis'->'granger': Must be a dict or bool.  Value provided is "
                            + str(granger)
                            + "."
                        )
                    elif isinstance(granger, dict):
                        ...
                        if not all(x in validList for x in list(granger.keys())):
                            errorMessages.append(
                                "SimConfig->'analysis'->'granger': granger must be a bool or dict with keys in list "
                                + str(validList)
                                + ". Keys supplied are "
                                + str(list(granger.keys()))
                                + "."
                            )

                        if 'cells1' in granger and not isinstance(granger['cells1'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'granger'->'cells1': Must be a list. Value provided is "
                                + str(granger['cells1'])
                                + "."
                            )
                        ...
                        if 'cells2' in granger and not isinstance(granger['cells2'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'granger'->'cells2': Must be a list. Value provided is "
                                + str(granger['cells2'])
                                + "."
                            )
                        ...
                        if 'spks1' in granger and not isinstance(granger['spks1'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'granger'->'spks1': Must be a list. Value provided is "
                                + str(granger['spks1'])
                                + "."
                            )
                        ...
                        if 'spks2' in granger and not isinstance(granger['spks2'], list):
                            errorMessages.append(
                                "SimConfig->'analysis'->'granger'->'spks2': Must be a list. Value provided is "
                                + str(granger['spks2'])
                                + "."
                            )
                        ...
                        if 'binSize' in granger and not isinstance(granger['binSize'], int):
                            errorMessages.append(
                                "SimConfig->'analysis'->'granger'->'binSize': Must be an int. Value provided is "
                                + str(granger['binSize'])
                                + "."
                            )
                        ...
                        if 'plotFig' in plot2DNet and not isinstance(plot2DNet['plotFig'], bool):
                            errorMessages.append(
                                "SimConfig->'analysis'->'plot2DNet'->'plotFig': Must be boolean. Value provided is "
                                + str(plot2DNet['plotFig'])
                                + "."
                            )
...
class TestObj(object):
    ...
    def __init__(self):
        ...
        self.testParameterValue1 = (
            ''  # test parameter value, like 'shape'. Used for nested dicts like cellParams['conds']['cellType']
        )
...
class SimTestObj(object):
    ...
    def loadSimConfigTests(self):
        ...
        testObj.messageText = [
            "simConfig->'recordTraces':recordTraces is not a dict.",
            "simConfig->'recordTracesTest':is not a valid value. Valid values are 'netParams', 'netCells', 'netPops', 'simConfig', 'simData'.",
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testValueList = ['net', 'netParams', 'netCells', 'netPops', 'simConfig', 'simData']
        testObj.messageText = [
            "simConfig->'saveDataInclude':saveDataInclude is not a dict.",
            "SimConfig->'saveDataInclude':is not a valid value. Valid values are 'netParams', 'netCells', 'netPops', 'simConfig', 'simData'.",
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
    ...
    def loadPopTests(self):
        ...
        testObj.messageText = [
            "popParams: One of the following must be specified in parameters: " + str(testObj.testParameterValueList)
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.messageText = ["popParams->'xnormRange': XNormRange invalid range."]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_VALID_RANGE]
        testObj.testValueRange = "[0,1]"
        testObj.messageText = ["popParams->'ynormRange': YNormRange invalid."]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_VALID_RANGE]
        testObj.testValueRange = "[0,1]"
        testObj.messageText = ["popParams->'znormRange': ZNormRange invalid."]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
    ...
    def loadNetTests(self):
        ...
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        testObj.messageText = [
            "NetParams->'sizeX': Value should be a float.",
            "NetParams->'sizeX': sizeX is not greater than 0.",
        ]
        ...
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        testObj.messageText = [
            "NetParams->'sizeY': Value should be a float.",
            "NetParams->'sizeY': sizeY is not greater than 0.",
        ]
        ...
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        testObj.messageText = [
            "NetParams->'sizeZ': Value should be a float.",
            "NetParams->'sizeZ': sizeZ is not greater than 0.",
        ]
    ...
    def loadCellTests(self):
        ...
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        testObj.messageText = [
            "cellParams->'conds': Conds does not exist.",
            "cellParams->'conds': Conds is not a dict.",
        ]
        ...
        testObj.messageText = [
            "cellParams->'cellType': cellType does not match the cellType specified in pop parameters."
        ]
        ...
        testObj.compareDict = "self.netParams.popParams"
        testObj.messageText = [
            "cellParams->'cellModel': cellModel does not match the cellModel specified in pop parameters."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_WARNING]
    ...
    def loadConnTests(self):
        ...
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        testObj.messageText = [
            "ConnParams->'preConds': Preconds does not exist.",
            "ConnParams->'preConds': Preconds is not a dict.",
        ]
        ...
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        testObj.messageText = [
            "ConnParams->'postConds': Postconds does not exist.",
            "ConnParams->'postConds': Postconds is not a dict.",
        ]
        ...
        testObj.testTypes = [TEST_TYPE_EXISTS, TEST_TYPE_IS_DICT]
        testObj.messageText = [
            "ConnParams->'secs': Secs, is specified, needs to be a dict.",
            "ConnParams->'secs': Secs is not specified. Will use 'soma' by default otherwise first available section.",
        ]
        ...
        testObj.testTypes = [TEST_TYPE_VALID_SEC_LIST]
        testObj.messageText = [
            "If synsPerConn > 1, a list of sections or sectionList can be specified. These secs need to be specified in the cell parameters."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
            ...
            if testType == TEST_TYPE_EXISTS:
                        ...
                        try:
                            ...
                            if self.verboseFlag:
                                print(
                                    (
                                        "Test: "
                                        + str(testObj.testParameterValue)
                                        + " for: "
                                        + str(testType)
                                        + " value: "
                                        + str(paramValues)
                            ...
                            print(
                                str(testObj.errorMessageLevel[testIndex])
                                + ": "
                                + str(testObj.messageText[testIndex] + ".Values provided are: " + str(paramValues))
                            )
                else:
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            ...
                            if (
                                testObj.testParameterValue in paramValues
                                and testObj.testParameterValue1 in paramValues[testObj.testParameterValue]
                                and paramValues[testObj.testParameterValue][testObj.testParameterValue1]
                            ):
                                self.testTypeObj.testExistsInDict(
                                    paramValues[testObj.testParameterValue][testObj.testParameterValue1],
                                    eval(testObj.compareDict),
                                    testObj.testParameterValue1,
                                )
                                if self.verboseFlag:
                                    print(
                                        (
                                            "Test: "
                                            + str(paramValues[testObj.testParameterValue][testObj.testParameterValue1])
                                            + " for: "
                                            + str(testType)
                                            + " value: "
                                            + str(eval(testObj.compareDict))
                                        )
                                    )
                                    print("PASSED")
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            self.testTypeObj.testInRange(
                                testObj.testParameterValue, eval(testObj.testValueRange), paramValues
                            )
                            if self.verboseFlag:
                                print(
                                    (
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            testParamValue = self.testTypeObj.testArrayInRange(
                                testObj.testParameterValue, eval(testObj.testValueRange), paramValues
                            )
                            if self.verboseFlag:
                                print(
                            ...
                            print(
                                str(testObj.errorMessageLevel[testIndex])
                                + ": "
                                + str(testObj.messageText[testIndex] + " Value = " + str(paramValue))
                            )
                    ...
                    try:
                        ...
                        try:
                            print(
                                str(testObj.errorMessageLevel[testIndex])
                                + ": "
                                + str(testObj.messageText[testIndex])
                                + " Value specified is "
                                + str(paramName)
                                + "."
                                + ".Values provided are: "
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            ...
                            print(
                                str(testObj.errorMessageLevel[testIndex]) + ": " + str(testObj.messageText[testIndex])
                            )
                else:
                    try:
                        paramName = ''
            ...
            #                 print str(testObj.errorMessageLevel[testIndex]) + ": " + str(testObj.messageText[testIndex]) + str(e[0])
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            if testObj.testParameterValue in paramValues:
                                self.testTypeObj.testIsValueList(
                                    paramValues[testObj.testParameterValue], testObj.testValueList
                                )
                                if self.verboseFlag:
                                    print(
                                        (
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            if testObj.testParameterValue in paramValues:
                                self.testTypeObj.testIsAllValueList(
                                    paramValues[testObj.testParameterValue], testObj.testValueList
                                )
                                if self.verboseFlag:
                                    print(
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            self.testTypeObj.testExistsInAllDicts(
                                paramValues, testObj.testParameterValue, testObj.testParameterDictString
                            )
                    ...
                    if self.verboseFlag:
                        print(
                            (
                                ...
                                + str(testType)
                                + " value: "
                                + str(testObj.testParameterValueList)
                            )
                        )
                    print(
                        str(testObj.errorMessageLevel[testIndex]) + ": " + str(testObj.messageText[testIndex]) + str(e)
                    )
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            errorMessages, mechsValidFlagList, mechsWarningFlagList = self.testTypeObj.testValidMechs(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            print(
                                (
                                    str(MESSAGE_TYPE_ERROR)
                                    + ": Pointps in cell params specified is invalid. Please check against utils.mechVarlist."
                                )
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            errorMessage = self.testTypeObj.testExistsInPop(
                                testObj.testParameterValue, paramValues, self.netParams.popParams
                            )
