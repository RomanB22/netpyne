    ...
    def createNEURONObj(self, prop=None):
        ...
        if self.tags['cellModel'] == 'NetStim':
            ...
            if 'seed' not in self.params:
                self.params['seed'] = sim.cfg.seeds[
                    'stim'
                ]  # note: random number generator initialized from sim.preRun()
        ...
        if self.tags['cellModel'] == 'VecStim':
            ...
            if 'interval' in self.params:
                ...
                fixedInterval = np.full(
                    int(((1 + 1.5 * noise) * sim.cfg.duration / interval)), [(1.0 - noise) * interval]
                )  # generate 1+1.5*noise spikes to account for noise
                ...
                if noise == 0.0:
                    ...
                    if numSpks < 100:
                        ...
                        spkTimes = np.cumsum(fixedInterval + np.array(negexpInterval)) + (
                            start - interval * (1 - noise)
                        )
                        ...
                        print(
                            '\nError: exceeded the maximum number of VecStim spikes per cell (%d > %d)'
                            % (numSpks, maxReproducibleSpks)
                        )
                        return
            ...
            if 'pulses' in self.params:
                for ipulse, pulse in enumerate(self.params['pulses']):
                    ...
                    if 'rate' in pulse and isinstance(pulse['rate'], list) and len(pulse['rate']) == 2:
                        rand = h.Random()
                        rand.Random123(
                            sim.hashStr('point_rate_pulse' + str(ipulse)), self.gid, sim.cfg.seeds['stim']
                    ...
                    if any([x not in pulse for x in ['start', 'end']]):
                        ...
                        # fixed interval of duration (1 - noise)*interval
                        fixedInterval = np.full(
                            int(((1 + 1.5 * noise) * (end - start) / interval)), [(1.0 - noise) * interval]
                        )  # generate 1+0.5*noise spikes to account for noise
                        numSpks = len(fixedInterval)
                        ...
                        if noise == 0.0:
                            ...
                            for i in range(numSpks):
                                vec.setrand(rand)
                                negexpInterval.append(vec.x[0])  # = np.array(vec)[0:len(fixedInterval)]
                            pulseSpikes = np.cumsum(fixedInterval + np.array(negexpInterval)) + (
                                start - interval * (1 - noise)
                            )
        ...
        for i in range(params['synsPerConn']):
            ...
            if sim.cfg.createNEURONObj:
                ...
                if self.conns[-1].get('plast'):
                    self._addConnPlasticity(self.conns[-1], netcon, weightIndex)
